...
def get_full_db():
    ...
    countries = ["意大利", "日本", "美国", "英国", "法国", "德国", "澳大利亚", "新加坡", "泰国", "马来西亚", "韩国", "加拿大", "俄罗斯", "越南", "新西兰", "瑞士", "荷兰", "西班牙", "希腊", "阿联酋", "土耳其", "菲律宾", "印度", "印尼", "埃及", "南非", "瑞典", "奥地利", "葡萄牙", "丹麦", "比利时", "捷克", "匈牙利", "冰岛", "芬兰", "波兰", "爱尔兰", "以色列", "柬埔寨", "缅甸", "老挝", "文莱", "沙特", "卡塔尔", "尼泊尔", "斯里兰卡"]
    return {c: {"旅游签": {"在职人员": base + fin + ["在职证明(盖章)", "营业执照副本(盖章)"]}} for c in countries}

VISA_DB = get_full_db()
CURRENCIES = {"CNY":"人民币", "USD":"美元", "EUR":"欧元", "GBP":"英镑", "JPY":"日元", "HKD":"港币", "AUD":"澳元", "THB":"泰铢", "SGD":"新币", "MYR":"林吉特"}
CITIES = {"北京":"Asia/Shanghai", "伦敦":"Europe/London", "巴黎":"Europe/Paris", "纽约":"America/New_York", "东京":"Asia/Tokyo", "悉尼":"Australia/Sydney"}

# ==========================================
# 4. PDF 生成逻辑 (解决 bytearray 报错)
# ==========================================
...
menu = st.sidebar.radio("核心功能", ["🛂 签证清单", "💱 汇率换算", "⏰ 时差查询"])
...
if menu == "🛂 签证清单":
    ...
    try:
        # 调用生成函数
        pdf_bytes = generate_pdf(f"{country}签证清单", data)
        st.download_button(
            label="📥 一键下载材料清单", 
            data=pdf_bytes, 
    ...
    try:
        ...
        with col1: amt = st.number_input("人民币 (CNY)", value=100.0)
        with col2: target = st.selectbox("目标货币", sorted(list(CURRENCIES.keys())), format_func=lambda x: f"{x}-{CURRENCIES[x]}")
